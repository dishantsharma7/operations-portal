...
const RegisterWriterCompanyMethod = async (writerCompanyRegistrationData: writerCompanyRegisterInterface) => {
  ...
  const hashedpass: string = await hashPassword(writerCompanyRegistrationData.password);
