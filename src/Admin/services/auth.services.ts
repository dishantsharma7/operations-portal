...
const createPermissionService = async (permissionData: createAdminAccessPermissionsInterface) => {
  if (!permissionData?.accessPermissionName) {
    throw new BadRequestError('Invalid input');
  }
  const createPermissionResponse = await authMethods.createPermissionMethod(permissionData);
...
// Admin Users
const adminRegisterService = async (adminRegistrationData: adminRegisterInterface) => {
  ...
  if (adminRegistrationData.dateOfBirth) {
    const dateOfbirth = new Date(adminRegistrationData.dateOfBirth).toISOString();
    adminRegistrationData.dateOfBirth = dateOfbirth;
  }

  const registrationResponse = await authMethods.RegisterAdminMethod(adminRegistrationData);
...
export { createPermissionService, createRoleService, getAllPermissionService, getAllRoleService, adminRegisterService, adminLoginService };
