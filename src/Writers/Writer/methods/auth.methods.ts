...
const RegisterWriterMethod = async (
  writerRegistrationData: writerRegisterInterface
) => {
  console.log(writerRegistrationData, "writerRegistrationData");
  const existingUsername = await prisma_client.writer_user.findFirst({
  ...
  const hashedpass: string = await hashPassword(
    writerRegistrationData.password
  );
  const registeredUser = await prisma_client.writer_user.create({
